    ...
    if not b2_match:
        ...
        for k in range(B2_pyace.shape[1]):
            if not np.allclose(B2_pyace[:, k], B2_manual[:, k], atol=1e-8):
                print(f"  MISMATCH at column {k}: pyace={B2_pyace[0,k]:.8f}, manual={B2_manual[0,k]:.8f}")
    ...
    for n1 in range(NMAX):
        for n2 in range(n1, NMAX):
            for l in range(LMAX + 1):
                ...
                if not match:
                    print(f"  MISMATCH n1={n1+1},n2={n2+1},l={l}: "
        ...
        for n1 in range(NMAX):
            for n2 in range(n1, NMAX):
                ...
                if not np.isclose(b2_l0, expected, atol=1e-12):
                    print(f"  r={r}: B2({n1+1},{n2+1},0)={b2_l0:.8f} != "
...
def test_pyace_energy_matches_lammps():
    ...
    ace_file = os.path.join(os.path.dirname(__file__), '..', 'examples', 
                            'Cu_npj_CompMat2021(1).ace')
...
def test_pyscal3_self_consistency():
    ...
    desc = pyscal3.ace(fcc, nmax=NMAX, lmax=LMAX, nu_max=2, 
    ...
    # All atoms identical
    all_same = all(np.allclose(B1[0], B1[i]) and np.allclose(B2[0], B2[i]) 
                   for i in range(1, 4))
    ...
    for n1 in range(NMAX):
        for n2 in range(n1, NMAX):
            ...
            if not np.isclose(b2_l0, expected, rtol=1e-10):
                print(f"  B2({n1},{n2},0)={b2_l0:.8f} != B1({n1})*B1({n2})={expected:.8f}")
                b1_b2_match = False
            col += 1
...
def test_pyscal3_rotation_invariance():
    ...
    # Original
    pyscal3.find_neighbors(fcc, method='cutoff', cutoff=CUTOFF)
    desc1 = pyscal3.ace(fcc, nmax=NMAX, lmax=LMAX, nu_max=2, 
                        cutoff=CUTOFF, normalize=False)
    ...
    desc2 = pyscal3.ace(fcc_rot, nmax=NMAX, lmax=LMAX, nu_max=2, 
                        cutoff=CUTOFF, normalize=False)
    ...
    assert match, f"pyscal3 not rotation invariant! nu1 diff={diff_nu1}, nu2 diff={diff_nu2}"
    print("PASSED")
    return True
